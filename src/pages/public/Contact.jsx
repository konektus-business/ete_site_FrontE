...
const MailIcon = ({ className = "size-[11.667px]" }) => (
  <div className={`relative ${className}`}>
    <img alt="" className="absolute inset-0 block size-full max-w-none" src={img.mailIcon} />
  </div>
);
...
const SocialProofPill = () => (
  <div className="flex h-[62px] w-[307px] items-center rounded-full border border-white/10 bg-white/10 pl-5 pr-4 backdrop-blur-md">
    <div className="flex items-center">
      {[img.user1, img.user2, img.user3].map((u, i) => (
        <img
          key={i}
          alt=""
          src={u}
          className={`-mr-3 size-10 rounded-full border-2 border-[#0b3f34] object-cover ${i === 2 ? "-mr-0" : ""}`}
        />
      ))}
    </div>
    <p className="ml-4 text-sm font-medium tracking-[0.35px] text-white">
      +500 entreprises nous<br />font confiance
    </p>
  </div>
);
...
const ContactCard = ({ icon, iconBg, title, description, stats, buttonText, buttonBg, buttonShadow }) => (
  <GlassCard className="w-[560px]">
    <div className={`flex size-14 items-center justify-center rounded-3xl ${iconBg}`}>
      <img alt="" src={icon} className="h-[22.5px] w-[25px]" />
    </div>
    <div className="flex flex-1 flex-col gap-8">
        ...
        <div className="flex items-center gap-4 pt-2 text-sm font-medium text-[#64748b]">
          {stats.map((stat, i) => (
            <span key={i} className="flex items-center gap-2">
              <img alt="" src={stat.icon} className="h-[11.7px] w-auto" />
              {stat.label}
            </span>
          ))}
        </div>
      ...
      <button
        className={`flex h-14 w-full items-center justify-center rounded-3xl px-8 py-4 text-base font-bold text-white transition-colors ${buttonBg} ${buttonShadow}`}
      >
        {buttonText === "Envoyez-nous un courriel" ? <MailIcon /> : null}
        {buttonText}
      </button>
    </div>
  </GlassCard>
  ...
  <section className="relative z-10 mx-auto flex w-[1248px] flex-col items-center gap-[46px] pt-[206px]">
    ...
    <div className="flex items-center gap-8">
      <ContactCard
        ...
        stats={[
          { icon: img.clockIcon, label: "< 2h Response" },
          { icon: img.globeIcon, label: "Global Support" },
        ]}
      ...
      <ContactCard
        ...
        stats={[
          { icon: img.boltIcon, label: "15m Avg Resolution" },
          { icon: img.starIcon, label: "Expert Level 3" },
        ]}
...
const OfficeCard = ({ image, imageAlt, country, city, address, phone }) => (
  <div className="relative h-[406px] w-[389px] overflow-hidden rounded-3xl border border-white/70 shadow-[0px_4px_15px_-1px_rgba(0,0,0,0.1),0px_10px_30px_-3px_rgba(30,179,148,0.1),0px_20px_40px_-5px_rgba(0,0,0,0.05)]">
    ...
    <div className="relative flex h-full flex-col gap-4 p-4">
      <div className="h-48 w-full overflow-hidden rounded-3xl bg-slate-100">
        <img alt={imageAlt} src={image} className="size-full object-cover opacity-90" />
      </div>
...
const OfficesSection = () => {
  const offices = [
    { image: img.officeTunisia, alt: "Tunis, Tunisie", country: "Tunisie", city: "Tunis", address: "Centre Urbain Nord", phone: "+216 21 000 200" },
    { image: img.officeAlgeria, alt: "Alger, Algérie", country: "Algérie", city: "Hydra, Alger", address: "12 Rue des Jardins", phone: "+213 21 000 000" },
    { image: img.officeEgypt, alt: "Le Caire, Égypte", country: "Egypte", city: "Cairo, Egypt", address: "11 Kasr Al Ainy", phone: "+20 125 478 587" },
  ];
  return (
...
const CTABanner = () => (
  <section
    ...
    style={{ backgroundImage: "linear-gradient(145deg, rgb(11,63,52) 0%, rgb(30,179,148) 100%)" }}
  >
