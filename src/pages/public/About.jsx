...
// `role` picks the card's image-fit variant ("Direction" vs "Marketing").
const AHMED = { name: "Ahmed Youssef", role: "Direction", title: "Directeur Général", bio: "Pilote la stratégie et l'entreprise.", image: imgTeamDirecteur, linkedin: "#", website: "#" };
const SALMA = { name: "Salma Bouzid", role: "Marketing", title: "Responsable Marketing", bio: "Construit la marque et la croissance.", image: imgTeamMarketing, linkedin: "#", website: "#" };
const TEAM = [AHMED, SALMA, AHMED, SALMA];
...
// Mission slider constants (from Figma motion export, node 1:11237 / 1:11236):
// each step slides the track -448px using this exact bezier over 1.2s.
const CARD_STEP = 448;
const MISSION_EASE = [0.782000720500946, 0.012000122107565403, 0.17400024831295013, 0.996000349521637];
const LOOP_COPIES = 9; // buffer copies so looping never runs out
...
// Real wave-line geometry from Vector.svg (viewBox 0 0 1440 501).
const WAVE_PATHS = [
...
// Each spark shrinks + drops onto the flat middle stretch, then grows back on the far curve.
const SPARK_ANIMS = [
...
function MissionCard({ mission }) {
  return (
    <div className="relative h-[380px] w-full shrink-0 overflow-hidden rounded-[30px] sm:h-[412px] sm:w-[447px] sm:flex-none sm:min-w-[447px] group">
      <img src={mission.image} alt={mission.title} className="absolute inset-0 h-full w-full object-cover transition-all duration-700 ease-in-out group-hover:blur-sm" />
      ...
      <div className="absolute inset-x-0 bottom-0 flex flex-col gap-2 p-8 text-white">
        ...
        <p className={`${F} text-base leading-5 text-white/90`}>{mission.description}</p>
      </div>
    </div>
  );
}

function ValueCard({ value, className = "" }) {
  return (
    <div className={`relative flex w-full max-w-[336px] flex-col gap-8 overflow-hidden rounded-xl bg-white px-6 py-8 shadow-[0px_12px_40px_0px_#126b59,0px_0px_0px_4px_white] ${className}`}>
      <div className="flex size-14 items-center justify-center rounded-xl bg-[#006b57]/10">
...
// Two-stage reveal: hover shows name + title; clicking the arrow expands
// the card to also show the bio + LinkedIn/website icons.
  ...
  return (
    ...
    >
      <img
        src={member.image}
        alt={member.name}
        className={isMarketing ? "w-[226px] h-[321px] aspect-[69/98] object-cover" : "absolute inset-0 h-full w-full object-cover"}
      />

      <div
        className={`pointer-events-none absolute inset-x-0 bottom-0 opacity-0 transition-all duration-300 ease-out group-hover:opacity-100 ${expanded ? "h-full" : "h-1/2"}`}
        style={{
          background: expanded
            ? "linear-gradient(to top, rgba(0,0,0,0.9) 0%, rgba(0,0,0,0.65) 45%, rgba(0,0,0,0) 80%)"
      ...
      <div className="absolute inset-x-0 bottom-0 translate-y-2 p-4 opacity-0 transition-all duration-300 ease-out group-hover:translate-y-0 group-hover:opacity-100">
        <div className="flex items-center justify-between gap-2">
          ...
          <button
            type="button"
            aria-label={expanded ? "Réduire les détails" : "Voir plus de détails"}
...
function RollingDigit({ digit, start, height, width, delay = 0 }) {
  ...
  return (
    <span className="relative inline-block overflow-hidden" style={{ height, width }}>
      <motion.span
        ...
        transition={{ y: { duration: 1.3, delay, ease: [0.16, 1, 0.3, 1] }, filter: { duration: 1.3, delay, times: [0, 0.4, 1] } }}
      >
        {sequence.map((d, i) => (
          <span key={i} className="flex items-center justify-center" style={{ height, width }}>{d}</span>
        ))}
      </motion.span>
    </span>
  );
...
function RollingNumber({ value, suffix, fontSize, digitHeight, digitWidth, start, baseDelay = 0 }) {
  return (
    <span className={`inline-flex items-end ${F} font-medium leading-none tracking-[-0.5px] text-[#0b3f34] tabular-nums ${fontSize}`}>
      {String(value).split("").map((ch, i) => (
        <RollingDigit key={i} digit={Number(ch)} start={start} height={digitHeight} width={digitWidth} delay={baseDelay + i * 0.12} />
      ))}
...
// Traces the ∞ loop once as a single continuous stroke (no subpath breaks,
// which some browsers reset the dash pattern on).
function InfinityIcon({ className = "", start }) {
  const d = "M12 12c-2-2.67-4-4-6-4s-4 1.33-4 4 1.33 4 4 4 4-1.33 6-4c2 2.67 4 4 6 4s4-1.33 4-4-1.33-4-4-4-4 1.33-6 4";
  return (
    <div className={`relative ${className}`} aria-hidden="true">
      <svg viewBox="0 0 24 24" fill="none" className="h-full w-full">
        {start && (
          <motion.path
            d={d} stroke="#0b3f34" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
            pathLength={1} strokeDasharray="1 1"
            initial={{ strokeDashoffset: 1 }} animate={{ strokeDashoffset: 0 }} transition={{ duration: 2, ease: "easeInOut" }}
          />
        )}
      </svg>
    </div>
  ...
  return (
    ...
    >
      <div className="flex h-16 items-end justify-center">
        {stat.value == null ? (
          <InfinityIcon className="h-16 w-16" start={start} />
        ) : (
          <RollingNumber {...stat} start={start} baseDelay={index * 0.15 + 0.2} />
        )}
      </div>
      <span className={`${F} text-base font-normal tracking-[-0.24px] text-[#0b3f34]`}>{stat.label}</span>
    </motion.div>
  );
}

// Sparks ride the real curve geometry via native SVG <animateMotion> (SMIL);
// rotate="auto" tilts each spark to the curve's tangent. Base lines are
// already baked into imgStatsBg — this layer only adds the moving sparks.
      ...
      {start &&
        WAVE_PATHS.filter((_, i) => i !== 2).map((d, i) => (
          <rect key={i} y="-1.9" height="2.6" rx="1.3" fill="url(#spark-fade)">
            ...
            {SPARK_ANIMS.map((a) => (
              <animate key={a.attr} attributeName={a.attr} keyTimes="0; 0.33; 0.67; 1" values={a.values} dur="4s" begin={`${i * 0.35}s`} repeatCount="indefinite" />
            ))}
          </rect>
        ))}
    </svg>
...
export default function About() {
  // Unbounded counter over a looped track (9 copies of the 3 missions) so
  // clicking prev/next always slides the same direction, never snapping back.
  ...
  // Silently re-center once we drift too far into the buffer.
  ...
  return (
    <div className="relative w-full overflow-hidden bg-[#ebf8f5] bg-cover bg-top bg-no-repeat" style={{ backgroundImage: `url(${bgImage})` }}>
      ...
      <div className="mx-auto flex max-w-[1440px] flex-col px-6 pt-[180px] sm:px-12 sm:pt-[200px] lg:px-24">
        <section className="flex flex-col items-center gap-12 lg:flex-row lg:justify-between lg:gap-16">
          ...
          <div className="relative flex shrink-0 items-center justify-center size-[376px] max-w-full">
      ...
      <section ref={statsRef} className="relative mt-16 flex w-full min-h-[480px] items-center justify-center overflow-hidden py-24 sm:py-32">
        ...
        <div className="relative z-10 mx-auto flex w-full max-w-[900px] flex-wrap items-start justify-center gap-x-8 gap-y-10 px-6">
          {STATS.map((stat, i) => (
            <StatItem key={stat.label} stat={stat} start={isStatsInView} index={i} />
          ))}
        </div>
      </section>
      ...
      <div className="mx-auto flex max-w-[1440px] flex-col gap-32 px-6 pb-32 sm:px-12 lg:px-24 mt-16">
        ...
        <section className="flex flex-col items-center gap-12 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex w-full max-w-[549px] items-center justify-between gap-4">
            <button type="button" onClick={prevMission} aria-label="Mission précédente" className="cursor-pointer shrink-0">
              <img src={imgArrowLeft} alt="Précédent" className="h-6 w-6" />
            </button>

            <div className="relative h-[380px] w-full overflow-hidden rounded-[30px] sm:h-[412px] sm:w-[447px]">
              <motion.div
                className="flex h-full"
                animate={{ x: -missionIndex * CARD_STEP }}
            ...
            <button type="button" onClick={nextMission} aria-label="Mission suivante" className="cursor-pointer shrink-0">
              <img src={imgArrowRight} alt="Suivant" className="h-6 w-6" />
            </button>
          </div>
        </section>
        ...
        <section className="flex flex-col gap-16">
          <h2 className={`${F} text-3xl font-black leading-[1.5] text-[#0d5143] sm:text-4xl`}>Nos valeurs</h2>
        ...
        <section className="flex flex-col gap-11">
          ...
          <div className="flex flex-col items-end gap-9">
            <button type="button" className={`flex items-center gap-2 ${F} text-base font-semibold text-[#006b57] transition hover:text-[#126b59]`}>
        ...
        {/* CTA */}
        <section className="flex flex-col items-center gap-7 rounded-[64px] bg-gradient-to-br from-[#006b57] to-[#1eb394] px-8 py-16 text-center shadow-[0px_25px_50px_-12px_#0d5143] sm:px-24">
          ...
          <button type="button" className={`rounded-full bg-white px-10 py-5 ${F} text-lg font-bold text-[#0b3f34] shadow-[0px_8px_10px_rgba(0,0,0,0.25)] transition hover:bg-white/90 sm:text-xl`}>
            Contactez-nous
          </button>
        </section>
      </div>
