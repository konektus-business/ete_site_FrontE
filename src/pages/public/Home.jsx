...
const SERV_HOVER = { duration: 1.25, ease: [0.25, 0.1, 0.25, 1] };
...
const CTAButtons = () => (
  <div className="flex flex-col items-center gap-6 sm:flex-row">
    {["Contactez-Nous", "Démarrer l'essai gratuit"].map((label) => (
      <button
        key={label}
        className="w-[279px] rounded-full bg-white px-8 py-4 text-[18px] font-bold text-[#2B6859] shadow-[0_8px_10px_rgba(0,0,0,0.25)] transition hover:bg-gray-100"
      >
        {label}
      </button>
    ))}
...
const ValueCard = memo(({ icon: iconName, size, label, index, isVisible }) => {
  // Pre‑defined offset values for each index (moved outside)
...
const ServiceCard = memo(({ title, description, bg, layers }) => {
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="absolute inset-x-0 bottom-0 z-10 flex h-1/2 flex-col justify-center gap-[14px] bg-white/95 p-4 backdrop-blur-sm"
...
const TestimonialCard = memo(({ quote, avatar, name, role, quoteMark }) => (
  <Reveal className="flex-1">
    <div className="relative flex h-full flex-col gap-8 rounded-[24px] bg-white p-10 shadow-[0_20px_25px_-5px_rgba(226,232,240,0.5),0_8px_10px_-6px_rgba(226,232,240,0.5)] drop-shadow-[0_4px_10px_#1EB394]">
      {quoteMark && <img src={asset("quote.png")} alt="" className="absolute -top-6 left-10 h-12 w-12" loading="lazy" decoding="async" />}
...
// ========== Section Components (to isolate useInView) ==========
const HeroSection = () => {
  ...
  return (
    <div className="relative flex h-[941px] w-full flex-col items-center overflow-hidden bg-[#0A1A17] pt-[210px] pb-[260px]">
      <div className="pointer-events-none absolute inset-0" style={HERO_BG}>
        {heroOrbs.map((orb, i) => (
          <motion.div
            ...
            // Animate only when hero is in view
      ...
      >
        <motion.div
          variants={{ hidden: { opacity: 0, y: 120 }, visible: { opacity: 1, y: 0, transition: S } }}
        ...
        >
          ...
          />
          <PrimaryButton type="submit" className="px-4 py-2 text-sm font-medium">
            Accès gratuit
          </PrimaryButton>
        </motion.form>
      </motion.section>
  ...
  return (
    ...
    >
      <img
        ...
        decoding="async"
      />
      {/* Pulse dots – animation only when card is in view */}
      {[
        ...
        <motion.span
          ...
          initial={{ opacity: 0 }}
          // One combined animation: initial appear + infinite pulse
          animate={
            isCardInView
              ? {
...
const KonvictionsSection = () => {
  ...
  // Pre‑defined delays per title for the animation
  const konvictionDelays = { Kulture: 0, Katalyst: 0.4, Kality: 0.8 };
  const konvictionYOffsets = { Kulture: 552, Katalyst: 741, Kality: 752 };
  ...
  return (
    <section ref={konvictionsRef} className={`mx-auto w-full max-w-[1248px] px-4 py-[100px] ${areKonvictionsInView ? "overflow-visible" : "overflow-hidden"}`}>
      ...
      >
        ...
        <p className="max-w-[900px] text-xl leading-[35px] text-black">
          Kulture, Katalyst, Kality : Nos principes fondamentaux pour innover et réussir. Nous ne nous contentons pas de bâtir des
    ...
    <section className="relative mx-auto w-full max-w-[1248px] px-4 py-20">
      ...
      <div ref={valuesTextRef} className="mb-16 flex flex-col items-start gap-7 text-left">
        <div className="w-full max-w-[770px]">
          ...
          >
            Les plus grandes entreprises technologiques au monde ont bâti leur réussite sur un principe fondamental : le pouvoir du
            code ouvert. Chez KoneKtUS, nous partageons exactement cette philosophie.
          </motion.p>
        </div>
      </div>
      ...
      <div ref={valuesRef} className="grid grid-cols-2 gap-6 sm:grid-cols-3 lg:grid-cols-5">
        {values.map((v, i) => (
          <ValueCard key={v.label} {...v} index={i} isVisible={areValuesInView} />
        ))}
        <div className="flex justify-end sm:col-start-3 lg:col-start-5">
          <button className="flex items-center gap-1 rounded-full bg-[#1EB394] px-4 py-2 text-white transition hover:bg-[#178f76]">
            En savoir plus <img src={icon("righticon.png")} alt="" className="h-3 w-3" loading="lazy" decoding="async" />
          </button>
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          <h2 className="text-[36px] font-black leading-[1.5] text-[#0B3F34]">
            ...
            <span className="bg-gradient-to-r from-[#0B3F34] to-[#1DA588] bg-clip-text text-transparent">jamais l'inverse.</span>
          </h2>
          <p className="max-w-[619px] text-xl leading-[1.5] text-black">
            KonektUS est née d'une conviction simple : la technologie doit être ouverte, évolutive et au service des entreprises. Nous
            bâtissons aujourd'hui un écosystème ouvert, audité et continuellement amélioré, conçu avec et pour une communauté
            mondiale.
          </p>
          <PrimaryButton className="mt-2 flex items-center gap-1 px-4 py-2 text-sm font-bold">
            En savoir plus <img src={icon("arrow-right.svg")} alt="" className="h-3 w-3" loading="lazy" decoding="async" />
          </PrimaryButton>
        </motion.div>
        <motion.div
          initial={{ opacity: 0, x: 600 }}
        ...
        {featureStripItems.map((f) => (
          <div key={f.title} className="flex items-center gap-4">
            <div className="flex shrink-0 items-center justify-center rounded-[8px] bg-[#2DD4BF]/10 p-3">
              <img src={icon(f.icon)} alt="" className="h-8 w-8" loading="lazy" decoding="async" />
            </div>
...
const TestimonialsSection = () => (
  <section className="mx-auto w-full max-w-[1248px] px-4 py-20">
    ...
    <motion.div
      ...
      variants={{
        hidden: { opacity: 0 },
        visible: { opacity: 1, transition: { staggerChildren: 0.1, delayChildren: 0.1, ...S } },
      }}
    ...
    <>
      ...
      <div className="relative w-full overflow-hidden">
        <div className="pointer-events-none absolute inset-0 -z-10">
          <img src={asset("bg.png")} alt="" className="absolute left-0 top-0 w-full object-cover" loading="lazy" decoding="async" />
          <img
            src={asset("bg.png")}
            alt=""
