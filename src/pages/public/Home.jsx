...
// ── Assets (paths) ──
// We'll keep them as strings for brevity; in a real project you'd import them.
// For optimization, we could group them but we'll just use the paths directly.
...
const values = [
  ...
  { icon: "licence.svg", size: "h-[25px] w-[20]", label: "Zéro licence propriétaire" },
...
const testimonials = [
  {
    quote: "\"L'implémentation de Konektus a radicalement changé notre gestion des données. Nous avons gagné 40% d'efficacité opérationnelle en 6 mois.\"",
    avatar: "sarah-l.png",
    name: "Sarah Leclair",
  ...
  {
    quote: '"Un outil puissant mais surtout un accompagnement premium qui comprend les enjeux business réels des ETI modernes."',
    avatar: "marc-a.png",
    name: "Marc Aubert",
    role: "Directeur Digital, Global Group",
  },
  {
    quote: '"La connectivité native entre nos différents ERP et outils de communication via VTM est tout simplement bluffante."',
    avatar: "julie-d.png",
    name: "Julie Dupont",
...
const SectionHeader = ({ tag, title, description }) => (
  <div className="mb-[54px] flex flex-col items-start gap-[16px] text-left">
    {tag && <p className="text-[14px] font-[700] uppercase tracking-[1.4px] text-[#1EB394]">{tag}</p>}
    <h2 className="text-[36px] font-[900] leading-[45px] text-[#0B3F34]">{title}</h2>
    {description && <p className="max-w-[576px] text-[18px] leading-[24px] text-black">{description}</p>}
  </div>
);

const EnSavoirPlusButton = ({ className = "" }) => (
  <button
  ...
  >
    En savoir plus
    <img src="src/assets/righticon.png" alt="" className="h-[11.667px] w-[11.667px]" />
  </button>
);
...
const CTAButtons = () => (
  <div className="flex flex-col items-center gap-[24px] sm:flex-row">
    {["Contactez-Nous", "Démarrer l'essai gratuit"].map((label) => (
      <button
        key={label}
        className="w-[279px] rounded-full bg-white px-[32px] py-[16px] text-[18px] font-[700] text-[#2B6859] shadow-[0_8px_10px_rgba(0,0,0,0.25)] transition hover:bg-gray-100"
      >
        {label}
      </button>
    ))}
...
const OrbsBackground = () => (
  <div
    ...
    style={{
      background: "linear-gradient(131deg, #061C18 0%, #0B3F34 38%, #128A72 78%, #1EB394 100%)",
    }}
  >
...
// ── Konviction Card ──
const KonvictionCard = ({ n, icon, iconSize, title, tag, text, check, img }) => (
  <div className="relative flex-1 rounded-[24px] border border-[#1EB394]/35 bg-[#F6F8F8] p-[41px] shadow-[0_25px_50px_-12px_rgba(30,179,149,0.4)]">
    <span className="absolute right-[32px] top-[8px] text-[96px] font-[900] leading-none text-[#0F172A]/5">{n}</span>
    ...
    <div className="flex flex-col gap-[24px]">
      <div>
        ...
        <p className="text-[14px] font-[700] uppercase tracking-[1.4px] text-[#1EB394]">{tag}</p>
      </div>
      <p className="text-[16px] leading-[26px] text-[#475569]">{text}</p>
      <div className="flex items-center gap-[8px] pt-[16px]">
        <img src="src/assets/icons/check-icon.svg" alt="" className="h-[15px] w-[15px]" />
      ...
      <div className="h-[160px] w-full overflow-hidden rounded-[12px] bg-[#F1F5F9]">
        <img src={`src/assets/${img}`} alt={title} className="h-full w-full object-cover" />
      </div>
    </div>
  </div>
);
...
// ── Value Card ──
const ValueCard = ({ icon, size, label }) => (
  <motion.div
    initial="rest"
    animate="rest"
    whileHover="hover"
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <img src={layers[0].src} alt={title} className="absolute inset-0 h-full w-full object-contain" />
      </motion.div>
      <motion.div
      ...
      >
        <div>
          <h3 className="text-[18px] font-[700] leading-[24px] text-[#0D5143]">{title}</h3>
          <p className="mt-[6px] text-[13px] leading-[19px] text-[#64748B]">{description}</p>
        </div>
        <button className="flex items-center gap-[4px] text-[13px] font-[700] text-[#1EB394]">
          Voir plus
          <img src="src/assets/icons/arrow-right-sm.svg" alt="" className="h-[14px] w-[14px]" />
        </button>
      </motion.div>
    </motion.div>
  );
...
const TestimonialCard = ({ quote, avatar, name, role, quoteMark }) => (
  <div className="relative flex flex-1 flex-col gap-[32px] rounded-[24px] bg-white p-[40px] shadow-[0_20px_25px_-5px_rgba(226,232,240,0.5),0_8px_10px_-6px_rgba(226,232,240,0.5)] drop-shadow-[0_4px_10px_#1EB394]">
    {quoteMark && <img src="src/assets/quote.png" alt="" className="absolute -top-[24px] left-[40px] h-[48px] w-[48px]" />}
    ...
    <div className="flex items-center gap-[16px] border-t border-[#F1F5F9] pt-[25px]">
      <img src={`src/assets/${avatar}`} alt={name} className="h-[48px] w-[48px] rounded-full object-cover" />
      <div>
    ...
    {featureStripItems.map((f) => (
      <div key={f.title} className="flex items-center gap-[16px]">
        <div className="flex shrink-0 items-center justify-center rounded-[8px] bg-[#2DD4BF]/10 p-[12px]">
          <img src={`src/assets/icons/${f.icon}`} alt="" className="h-[32px] w-[32px]" />
        </div>
  ...
  return (
    <>
      ...
      <div className="relative flex h-[941px] w-full flex-col items-center overflow-hidden bg-[#0A1A17] pt-[210px] pb-[260px]">
        ...
        <section className="relative z-10 flex w-full max-w-[1248px] flex-col items-center gap-[56px] px-4">
          ...
          <form className="flex w-full max-w-[319px] items-center justify-between gap-2 rounded-full bg-white px-5 py-2.5">
            ...
            <button
              type="submit"
              className="shrink-0 whitespace-nowrap rounded-3xl bg-[#0D5143] text-sm font-medium leading-[1.4] text-white transition-colors hover:bg-[#0B3F34] px-4 py-2"
            >
              Accès gratuit
            </button>
          </form>
      ...
      <div className="relative w-full overflow-hidden">
        {/* Background image layers (static) */}
        <div className="pointer-events-none absolute inset-0 -z-10">
          <img src="src/assets/bg.png" alt="" className="absolute left-0 top-0 w-full object-cover" />
          <img src="src/assets/bg.png" alt="" className="absolute left-0 top-[1900px] w-full -scale-y-100 object-cover" />
          <img src="src/assets/bg.png" alt="" className="absolute left-0 top-[3800px] w-full object-cover" />
        </div>

        {/* Konvictions */}
        <section className="mx-auto w-full max-w-[1248px] px-4 py-[100px]">
          <div className="mb-[64px] flex flex-col gap-[28px]">
            <h2 className="text-[36px] font-[900] leading-[1.5] text-[#0B3F34]">Nos Konvictions</h2>
        ...
        <section className="relative mx-auto w-full max-w-[1248px] px-4 py-[80px]">
          <div className="mb-[64px] flex flex-col items-start gap-[28px] text-left">
            <div className="w-full max-w-[770px]">
              <p className="text-[14px] font-[700] uppercase tracking-[1.4px] text-[#1EB394]">Nos valeurs</p>
              <h2 className="text-[36px] font-[900] leading-[1.5] text-[#0B3F34]">Pourquoi choisir KonektUS ?</h2>
          ...
          <section className="mx-auto flex w-full max-w-[1248px] flex-col items-center gap-[48px] px-4 py-[80px] lg:flex-row">
            <div className="flex flex-1 flex-col items-start gap-[16px]">
              <p className="text-[14px] font-[700] uppercase tracking-[1.4px] text-[#1EB394]">Notre vision</p>
              ...
              <button className="mt-[8px] flex items-center gap-[4px] rounded-full bg-[#1EB394] px-[16px] py-[8px] text-[14px] font-[700] text-white transition hover:bg-[#178f76]">
                En savoir plus
                <img src="src/assets/icons/arrow-right.svg" alt="" className="h-[12px] w-[12px]" />
              </button>
            </div>
            <div className="flex-1">
              <img src="src/assets/vision-illustration.png" alt="Notre vision" className="w-full object-cover" />
            </div>
          </section>
        ...
        <section className="mx-auto w-full max-w-[1248px] px-4 py-[100px]">
          ...
          <div className="flex gap-[32px] overflow-x-auto pb-[16px] [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {services.map((service, index) => (
              <div key={service.title} className="flex shrink-0 flex-col items-end gap-[24px]">
                <ServiceCard {...service} />
        ...
        <section className="mx-auto w-full max-w-[1248px] px-4 py-[80px]">
          <h2 className="mb-[64px] text-center text-[36px] font-[800] text-[#0B3F34]">Approuvé par les leaders</h2>
