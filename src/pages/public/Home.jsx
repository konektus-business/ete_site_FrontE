...
const services = [
  {
    ...
    description: "Révolutionnez vos échanges avec une infrastructure voix robuste et flexible.",
    bg: "rgba(30,179,148,0.28)",
    layers: [asset("tel-voip.png"), asset("tel-voip-2.png")],
  },
  {
    title: "Cloud & Hébergement",
    description: "Propulsez vos applications sur une infrastructure cloud sécurisée.",
    bg: "rgba(30,179,148,0.28)",
    layers: [asset("cloud.png"), asset("cloud-frame-2147223592.png")],
  },
  {
    title: "Intégration & Conseil",
    description: "Analyse approfondie de votre infrastructure et roadmap stratégique personnalisée.",
    bg: "rgba(30,179,148,0.28)",
    layers: [asset("integration.png")],
  },
  {
    title: "Data & Analyse",
    description: "Transformez vos données en décisions stratégiques grâce à des pipelines robustes.",
    bg: "rgba(30,179,148,0.28)",
    layers: [asset("big-data.png")],
  },
  {
    title: "Support & Infogérance",
    description: "Surveillance continue de vos systèmes pour prévenir les incidents.",
    bg: "rgba(30,179,148,0.28)",
...
const KonvictionCard = memo(({ n, icon: iconName, iconSize, title, tag, text, check, img }) => (
  <div className="relative flex flex-1 flex-col rounded-[24px] border border-[#1EB394]/35 bg-[#F6F8F8] p-6 shadow-[0_25px_50px_-12px_rgba(30,179,149,0.4)] sm:p-8 lg:p-[41px]">
    <span className="absolute right-4 top-1 text-[56px] font-black leading-none text-[#0F172A]/5 sm:right-6 sm:text-[72px] lg:right-[32px] lg:top-[8px] lg:text-[96px]">{n}</span>
    <div className="mb-6 flex h-[56px] w-[56px] items-center justify-center rounded-[20px] bg-[#E9F7F4] sm:mb-8 sm:h-[64px] sm:w-[64px] lg:mb-[32px]">
      <img src={icon(iconName)} alt="" className={iconSize} loading="lazy" decoding="async" />
    </div>
    <div className="flex flex-1 flex-col gap-5 sm:gap-6">
      <div>
        <h3 className="text-xl font-extrabold text-[#0F172A] sm:text-[24px]">{title}</h3>
        <p className="text-xs font-bold uppercase tracking-[1.4px] text-[#1EB394] sm:text-sm">{tag}</p>
      </div>
      <p className="text-sm leading-6 text-[#475569] sm:text-[16px] sm:leading-[26px]">{text}</p>
      <div className="mt-auto flex items-center gap-2 pt-3 sm:pt-4">
        <img src={icon("check-icon.svg")} alt="" className="h-[15px] w-[15px]" loading="lazy" decoding="async" />
      ...
      <div className="h-[130px] w-full overflow-hidden rounded-[12px] bg-[#F1F5F9] sm:h-[150px] lg:h-[160px]">
        <img src={asset(img)} alt={title} className="h-full w-full object-cover" loading="lazy" decoding="async" />
      </div>
    </div>
  </div>
));
    ...
    >
      ...
      >
        <div className="flex h-[52px] w-[52px] items-center justify-center rounded-[16px] bg-[#B8EADA]/40 sm:h-[64px] sm:w-[64px]">
          <img src={icon(iconName)} alt="" className={size} loading="lazy" decoding="async" />
        </div>
...
const ServiceCard = memo(({ title, description, bg, layers }) => {
  const imgVariants = { rest: { height: "100%", transition: { duration: 1.25, ease: [0.25, 0.1, 0.25, 1] } }, hover: { height: "50%", transition: { duration: 1.25, ease: [0.25, 0.1, 0.25, 1] } } };
  const textVariants = { rest: { y: 20, opacity: 0 }, hover: { y: 0, opacity: 1, transition: { duration: 1.25, ease: [0.25, 0.1, 0.25, 1] } } };
  return (
    <motion.div
      ...
      whileTap="hover"
    >
      <div className="relative h-full w-full overflow-hidden rounded-[20px] bg-white/70 backdrop-blur-[6px]">
        <motion.div className="absolute inset-x-0 top-0 w-full overflow-hidden" style={{ backgroundColor: bg }} variants={imgVariants}>
          ...
          <img src={layers[0]} alt={title} className="absolute inset-0 h-full w-full object-contain" loading="lazy" decoding="async" />
        </motion.div>
        <motion.div className="absolute inset-x-0 bottom-0 z-10 flex h-1/2 flex-col justify-center gap-3 bg-white/95 p-3 backdrop-blur-sm sm:gap-[14px] sm:p-4" variants={textVariants}>
          <div>
            <h3 className="text-[16px] font-bold leading-6 text-[#0D5143] sm:text-[18px]">{title}</h3>
...
const TestimonialCard = memo(({ quote, avatar, name, role, quoteMark }) => (
  <motion.div
    ...
    className="flex-1"
  >
    <div className="relative flex h-full flex-col gap-6 rounded-[24px] bg-white p-6 shadow-[0_20px_25px_-5px_rgba(226,232,240,0.5),0_8px_10px_-6px_rgba(226,232,240,0.5)] drop-shadow-[0_4px_10px_#1EB394] sm:gap-8 sm:p-8 lg:p-10">
      {quoteMark && <img src={asset("quote.png")} alt="" className="absolute -top-5 left-6 h-10 w-10 sm:-top-6 sm:left-10 sm:h-12 sm:w-12" loading="lazy" decoding="async" />}
      <p className="flex-1 text-sm leading-6 text-[#475569] sm:text-[16px]">{quote}</p>
      <div className="mt-auto flex items-center gap-4 border-t border-[#F1F5F9] pt-5 sm:pt-[25px]">
        <img src={asset(avatar)} alt={name} className="h-12 w-12 rounded-full object-cover" loading="lazy" decoding="async" />
        <div>
...
const HeroSection = () => {
  ...
  return (
    <div className="relative flex min-h-[560px] w-full flex-col items-center overflow-hidden bg-[#0A1A17] px-4 pb-24 pt-28 sm:min-h-[720px] sm:pb-40 sm:pt-40 lg:h-[941px] lg:pb-[260px] lg:pt-[210px]">
        ...
        <div className="absolute inset-0 scale-[0.55] sm:scale-75 lg:scale-100">
          {heroOrbs.map((orb, i) => (
            <motion.div
              ...
              animate={isHeroInView ? { x: [0, orb.x], y: [0, orb.y] } : { x: 0, y: 0 }}
      ...
      >
        <motion.div
          variants={{ hidden: { opacity: 0, y: 60 }, visible: { opacity: 1, y: 0, transition: S } }}
        ...
        >
          ...
          <motion.p
            variants={{ hidden: { opacity: 0, y: -40 }, visible: { opacity: 1, y: 0, transition: { ...S, delay: 0.05 } } }}
          ...
          >
            <span className="block sm:inline">Simplifiez vos interactions, libérez vos équipes des complexités inutiles</span>{" "}
            <span className="block sm:inline">et concentrez-vous sur ce qui compte vraiment.</span>
          </motion.p>
        </motion.div>
        <motion.form
          variants={{ hidden: { opacity: 0, y: -80 }, visible: { opacity: 1, y: 0, transition: { ...S, delay: 0.1 } } }}
        ...
        >
          ...
          <button type="submit" className="rounded-full bg-[#1EB394] px-4 py-2 text-sm font-medium text-white transition hover:bg-[#178f76]">Accès gratuit</button>
        </motion.form>
      </motion.section>
    </div>
  );
      ...
      <div className="hidden sm:block">
        {[{ color: "#1EB394", top: "18%", left: "12%", delay: 0 }, { color: "#2E90FA", top: "34%", left: "68%", delay: 0.3 }, { color: "#F79009", top: "62%", left: "40%", delay: 0.6 }].map((dot, i) => (
          <motion.span
            ...
            initial={{ opacity: 0 }}
            animate={isCardInView ? { opacity: [0, 1, 0.3, 1, 0.3, 1], scale: [0.5, 1, 1.6, 1, 1.6, 1] } : { opacity: 0, scale: 0.5 }}
            transition={{ duration: 2, ease: "linear", repeat: isCardInView ? Infinity : 0, delay: dot.delay, times: [0, 0.15, 0.35, 0.55, 0.75, 1] }}
          />
        ))}
      </div>
      <motion.div
      ...
      >
        <div className="marquee-rtl flex w-max items-center gap-10 sm:gap-[90px]">
          {[...logos, ...logos].map((logo, i) => (
            <div key={`${logo.alt}-${i}`} className="flex h-[40px] w-[110px] shrink-0 items-center justify-center opacity-70 grayscale transition hover:opacity-100 hover:grayscale-0 sm:h-[55px] sm:w-[149px]">
              <img src={logo.src} alt={logo.alt} className="max-h-full max-w-full object-contain" loading="lazy" decoding="async" />
            </div>
          ))}
        </div>
...
const KonvictionsSection = () => {
  ...
  return (
    ...
    <section ref={konvictionsRef} className="mx-auto w-full max-w-[1248px] overflow-visible px-4 py-16 sm:py-20 lg:py-[100px]">
      ...
      >
        <h2 className="text-[26px] font-black leading-snug text-[#0B3F34] sm:text-[32px] lg:text-[36px] lg:leading-[1.5]">Nos Konvictions</h2>
        <p className="max-w-[900px] text-base leading-6 text-black sm:text-lg sm:leading-8 lg:text-xl lg:leading-[35px]">
          Kulture, Katalyst, Kality : Nos principes fondamentaux pour innover et réussir. Nous ne nous contentons pas de bâtir des outils, nous forgeons l'avenir du B2B.
        </p>
      </motion.div>
    ...
    <section className="relative mx-auto w-full max-w-[1248px] overflow-hidden px-4 py-14 sm:py-16 lg:py-20">
      ...
      <div ref={valuesRef} className="grid grid-cols-2 gap-4 sm:grid-cols-3 sm:gap-6 lg:grid-cols-5">
        ...
        <div className="col-span-2 flex justify-end sm:col-span-1 sm:col-start-3 sm:justify-end lg:col-start-5">
          <Link to="/about#valeurs" className="flex items-center gap-1 rounded-full bg-[#1EB394] px-4 py-2 text-sm text-white transition hover:bg-[#178f76]">
      ...
      >
        ...
        >
          <p className="text-xs font-bold uppercase tracking-[1.4px] text-[#1EB394] sm:text-sm">Notre vision</p>
          ...
          <p className="max-w-[619px] text-base leading-6 text-black sm:text-lg lg:text-xl lg:leading-[1.5]">
            KonektUS est née d'une conviction simple : la technologie doit être ouverte, évolutive et au service des entreprises. Nous bâtissons aujourd'hui un écosystème ouvert, audité et continuellement amélioré, conçu avec et pour une communauté mondiale.
          </p>
          <Link to="/about#mission" className="mt-2 flex items-center gap-1 rounded-full bg-[#1EB394] px-4 py-2 text-sm font-bold text-white transition hover:bg-[#178f76]">
        ...
        >
          <img src={asset("vision-illustration.png")} alt="Notre vision" className="w-full object-cover" loading="lazy" decoding="async" />
        </motion.div>
      </motion.section>
      <motion.div
        initial={{ opacity: 0, y: 100 }}
        ...
        className="mx-auto mt-10 grid max-w-[1024px] grid-cols-1 gap-6 rounded-[16px] border border-[#2DD4BF]/10 bg-[#0D5143]/80 p-5 shadow-[0_8px_32px_rgba(0,0,0,0.37)] backdrop-blur-[6px] sm:mt-16 sm:gap-8 sm:p-6 sm:grid-cols-3 lg:p-[33px]"
      >
        {featureStripItems.map(f => (
          <div key={f.title} className="flex items-center gap-4">
            <div className="flex shrink-0 items-center justify-center rounded-[8px] bg-[#2DD4BF]/10 p-3">
              <img src={icon(f.icon)} alt="" className="h-7 w-7 sm:h-8 sm:w-8" loading="lazy" decoding="async" />
            </div>
...
const ServicesSection = () => {
  ...
  const txtSpring = { type: "spring", mass: 1, stiffness: 100, damping: 15, visualDuration: 1.91645 };
  return (
    <section ref={servicesRef} className="mx-auto w-full max-w-[1248px] px-4 py-16 sm:py-20 lg:py-[100px]">
      <motion.div
      ...
      >
        <p className="text-xs font-bold uppercase tracking-[1.4px] text-[#1EB394] sm:text-sm">Expertise</p>
        <h2 className="text-[26px] font-black leading-[32px] text-[#0B3F34] sm:text-[30px] sm:leading-[38px] lg:text-[36px] lg:leading-[45px]">Nos services</h2>
        <p className="max-w-[576px] text-base leading-6 text-black sm:text-[18px]">Solutions de communication unifiées conçues pour l'excellence opérationnelle.</p>
      </motion.div>
      ...
      <div className="mt-6 flex justify-center pr-0 sm:mt-8 sm:justify-end sm:pr-4">
        <Link to="/services#services" className="flex items-center gap-1 rounded-full bg-[#1EB394] px-4 py-2 text-sm text-white transition hover:bg-[#178f76]">
          En savoir plus <img src={arrowRightIcon} alt="" className="h-3 w-3" />
        </Link>
      </div>
    </section>
...
const CtaSection = () => {
  ...
  return (
    <section ref={ctaRef} className="mx-auto w-full max-w-[1248px] px-4 py-10 sm:py-[60px]">
      <motion.div
      ...
      >
        ...
        <div className="flex w-full flex-col items-center gap-4 sm:w-auto sm:flex-row sm:gap-6">
          {["Contactez-Nous", "Démarrer l'essai gratuit"].map(label => (
            <button key={label} className="w-full max-w-[320px] rounded-full bg-white px-6 py-3 text-[15px] font-bold text-[#2B6859] shadow-[0_8px_10px_rgba(0,0,0,0.25)] transition hover:bg-gray-100 sm:w-[279px] sm:px-8 sm:py-4 sm:text-[18px]">
              {label}
            </button>
          ))}
        </div>
